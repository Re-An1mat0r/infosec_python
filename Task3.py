...
равняется 15, количество ячеек в ряду — 5. Тогда метод make_order() вернёт строку: *****\n*****\n*****.'''
...
class Cell:
    ...
    def __add__(self, other):
        return f'Сложение клеток, размер клетки равен {self.quantity + other.quantity}'

    def __sub__(self, other):
        sub = self.quantity - other.quantity
        if sub > 0:
            return f'Вычитание клеток, размер клетки равен {sub} клеткам'
        else:
    ...
    def make_order(self, cells_in_row):
        rows, remainder = divmod(self.quantity, cells_in_row)
        return '\n'.join(['*' * cells_in_row] * rows + (['*' * remainder] if remainder else []))
